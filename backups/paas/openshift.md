...
L'offre RedHat Openshift de Cloud Temple s'appuie sur une architecture composée d'Openshift et d'Openshift Data Foundation, répartie au sein de 3 zones de disponibilités dans la région FR1-PARIS.
...
Les prochaines fonctionnalités à venir dans l'environnement SecNumCloud incluent plusieurs améliorations: un certificat personnalisé sera appliqué au niveau de l'espace de noms, et KubeVirt sera intégré pour une meilleure gestion des machines virtuelles. 

Concernant les connexions TCP, une plage de ports par défaut (de 30000 à 32767) sera mise à disposition.

La console Cloud Temple sera utilisée pour l'administration des nœuds, avec des mises à jour prévues pour l'interface OpenShift. L'ajout de nœuds sera facilité par la commande 'Order', tandis que leur retrait passera par un ticket auprès des services professionnels. 

L'interconnexion des flux privés PaaS vers l'infrastructure IaaS de Cloud Temple sera également possible, améliorant la gestion des flux sortants OpenShift.

L'intégration OpenShift IAM avec la console Cloud Temple est prévue, tout comme la mise en place de Hapee (HAProxy) devant le Load Balancer public. Enfin, le support des GPU sera disponible dans OpenShift grâce à l'Openshift AI Operator et watsonx.ai, permettant des applications d'intelligence artificielle sur la plateforme.
